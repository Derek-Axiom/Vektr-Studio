    ...
    <div className="pb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex items-start gap-8">
            <div className="w-48 h-48 bg-gray-100 rounded-[2.5rem] border border-line flex flex-col items-center justify-center relative overflow-hidden group shadow-xl shadow-black/5">
              {activeItem.type === 'video' ? (
                <video src={activeItem.fileUrl} controls autoPlay muted className="absolute inset-0 w-full h-full object-contain bg-[var(--color-bg-panel)] z-10" />
                ...
                <>
                  ...
                  <Play className="w-12 h-12 text-[var(--color-text)] relative z-20 opacity-0 group-hover:opacity-100 transition-opacity" />
                </>
              )}
            </div>
            <div className="flex-1 pt-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[10px] font-black uppercase tracking-widest px-2 py-1 bg-[var(--color-bg-panel)] text-[var(--color-text)] rounded">
              ...
              <div className="flex gap-4 mt-8">
                {activeItem.type === 'audio' && (
                  <button onClick={() => setActiveMediaId(activeItem.id)} className="bg-[var(--color-bg-panel)] text-[var(--color-text)] px-8 py-3 rounded-xl font-bold hover:scale-105 transition-transform flex items-center gap-2 cursor-pointer shadow-xl">
          ...
          {isEditing && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <button onClick={() => setIsEditing(false)}
                className="w-full py-4 bg-[var(--color-bg-panel)] text-amber-400 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-[1.01] transition-all shadow-xl shadow-black/20 italic">
        ...
        <div className="w-full lg:w-80 space-y-6">
          <div className="bg-[var(--color-bg-panel)] text-[var(--color-text)] p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
            ...
            <p className="text-sm text-[var(--color-text)] mb-6 relative z-10">Apply premium mastering chains to this asset instantly.</p>
