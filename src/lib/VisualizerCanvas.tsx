...
import type { AudioAnalyzerData } from '../components/Modules/modules/audio/ART_HardwareFFT';
import { renderArtistBranding } from './ArtistBranding';
...
export const VisualizerCanvas = forwardRef<HTMLCanvasElement, Props>((props, ref) => {
  const { mode, style, audioData, lyrics = '', currentTime = 0, context, showIntegrityGhost = false, sovereignHash, isDemo = false, className } = props;
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const { mode: perfMode } = usePerformanceManager();
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (perfMode === 'emergency_killswitch') {
        ...
        if (ctx) {
           ...
           ctx.fillText('AUDIO PRIORITY LOCK', canvas.width/2, canvas.height/2);
           ctx.fillStyle = '#ef444455';
           ctx.font = '10px uppercase tracking-widest monospace';
           ctx.fillText('CANVAS PAUSED TO PREVENT DSP OVERLOAD', canvas.width/2, canvas.height/2 + 20);
        }
      ...
      const scale = perfMode === 'thermal_scaling' ? 1 : 2;
      ...
      // Layer: above background, below mode renders, below lyrics.
      // Crest (deterministic polygon) + Foundation (name arc) - audio-reactive.
      if (context?.username) {
        renderArtistBranding(ctx, context.username, {
          amplitude: audioData.amplitude,
          bass: audioData.bass,
        }, w, h);
      }

      // ── SOVEREIGN WATERMARK (PRE-VALIDATION INTEGRITY) ─────────────────
      if (sovereignHash) {
         ctx.save();
         ctx.textAlign = 'right';
         ctx.textBaseline = 'bottom';
         ctx.font = `bold ${Math.max(12, w * 0.015)}px monospace`;
         ctx.fillStyle = 'rgba(255,255,255,0.4)';
         if (audioData.treble > 0.8 && Math.random() > 0.8) {
             ctx.fillStyle = 'rgba(255,255,255,0.8)';
             ctx.shadowBlur = 10; ctx.shadowColor = '#fff';
         }
         ctx.fillText(`NFOD::Σ${sovereignHash.substring(0,16).toUpperCase()}`, w - 24, h - 24);
         ctx.restore();
      }
